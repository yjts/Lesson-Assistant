...
test("generation is deterministic and preserves unusual topic characters as text", () => {
  const input = { grade: "Grade 8", subject: "U.S. History", topic: "Rights & protest <script>alert('x')</script>", skill: "Evidence-Based Claims", standard: "CA HSS 8.1", duration: 45 };
  const first = generateLesson(input);
  const second = generateLesson(input);
  assert.deepEqual(first, second);
  assert.match(first.inquiryQuestion, /Rights & protest <script>alert\('x'\)<\/script>/);
});
